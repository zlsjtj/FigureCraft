...
6. 原生 SVG 或其他已有流程走[已有 SVG 检查](references/existing-svg-review.md)，检查辅助线与文字接触，并审阅说明文字所属区域；未纳入场景引擎的图不能借用它的 PASS。按实际入稿尺寸渲染与检查，查看正常色、灰度和选定色觉模拟。主标签以 9–10 pt 为起点；小于 8 pt 需要修复或记录未解决。不得靠缩小字体解决所有拥挤。缺字属于失败，字体必须可用。
