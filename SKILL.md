...
| 任务 | 处理方式 |
|---|---|
| 机制解释 | 先读[机制图的信息分工](references/visual-explanation.md)，选一个主动作并分配图内/图注/正文，再读[对象与深度](references/drawing-and-depth.md)组织构图 |
...
1. 机制图先选一个最需要看懂的动作，写出图内必须辨认的对象与变化；按[信息分工](references/visual-explanation.md)把次要条件分配到图注或正文，并声明本图文字预算。先用对象、分组、对应与箭头讲清主动作，再加少量必要标签；不要把所有已知关系挤进一张图。
...
6. 定稿前执行[减字与主动作复核](references/design-exploration.md)：必须有删去、合并或移出解释后仍能看懂的实际版本；压缩失败也保留理由。原生 SVG 或其他已有流程走[已有 SVG 检查](references/existing-svg-review.md)，检查辅助线与文字接触，并审阅说明文字所属区域；未纳入场景引擎的图不能借用它的 PASS。按实际入稿尺寸渲染与检查，查看正常色、灰度和选定色觉模拟。机制图主标签以 10–11 pt 为起点，其他图按对象密度与版式安排；小于 8 pt 需要修复或记录未解决。不得靠缩小字体解决所有拥挤。缺字属于失败，字体必须可用。
...
保留原稿，所有输出使用新路径。运行说明见 [README](README.md)，当前验收见[记录](provenance/acceptance-v1.10.md)，来源与许可见[说明](provenance/upstream-sources.md)。`status` 是兼容技术字段，科学或视觉审阅待定时 `overall_status` 不能为 PASS。审阅绑定实际文件哈希；未运行的路径明确记为 NOT_RUN，不承诺期刊接受。
